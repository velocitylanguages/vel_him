"""
Velocity Hindi - Unified Social Media Upload Script
...
def get_latest_reel():
    ...
    return {
        ...
        "metadata": metadata,
        "category": metadata.get("category_hindi", "Hindi Learning"),
...
def generate_caption(phrases, category, platform="facebook"):
    ...
    if platform == "facebook":
        ...
        caption_lines = [
            f"🇮🇳 Learn Hindi with Velocity Hindi! 🇮🇳",
            f"",
            f"📚 Category: {category}",
            f"",
            f"🎯 Master Hindi one phrase at a time! Today's {category} lesson:",
            f""
        ]
        ...
        for i, phrase in enumerate(phrases[:5], 0):
            ...
            caption_lines.append(f"   📍 {phrase['hindi']}")
        ...
        caption_lines.extend([
            ...
            f"🔔 Follow for daily Hindi lessons!",
            f"",
...
def upload_to_all_platforms(video_path, caption, category):
    ...
    print("\n" + "="*80)
    print("🚀 VELOCITY HINDI - MULTI-PLATFORM UPLOAD")
    print("="*80)
...
def main():
    ...
    print("\n" + "="*80)
    print("🇮🇳 VELOCITY HINDI - AUTOMATED UPLOAD 🇮🇳")
    print("="*80)

"""
Facebook Reels Automation - Bilingual English/Hindi Content Generator
IMPROVED VERSION: Better backgrounds, English categories, no repeats, Velocity Hindi branding
"""
...
# English category names (for English speakers learning Hindi)
CATEGORIES_ENGLISH = [
...
# Hindi translations for display
CATEGORIES_HINDI = {
    "Motivation": "प्रेरणा",
    "Love": "प्यार",
    "Success": "सफलता",
...
def generate_phrases(category_english: str, num_phrases: int = 5) -> list:
    ...
    for attempt in range(max_attempts):
        try:
            ...
            prompt = f"""Create {num_phrases * 2} unique {category_english} phrases for English speakers learning Hindi.
...
2. Hindi translation (in Devanagari script, with commas matching the rhythm)
...
IMPORTANT: Create FRESH, UNIQUE phrases that haven't been used before."""
            ...
            payload = {
                "model": AI_MODEL,
                "messages": [
                    {"role": "system", "content": "You are a Hindi teacher. Create short, natural phrases with pauses."},
...
def get_fresh_fallback_phrases(category: str, num_phrases: int) -> list:
    ...
    all_fallbacks = {
        "Motivation": [
            {"english": "Believe in yourself.", "hindi": "खुद पर भरोसा करो।", "pronunciation": "Khud par bharosa karo."},
            {"english": "You are capable of amazing things.", "hindi": "तुम अद्भुत चीजों में सक्षम हो।", "pronunciation": "Tum adbhut cheezon mein saksham ho."},
            {"english": "Dream big, start small.", "hindi": "बड़ा सपना देखो, छोटे से शुरू करो।", "pronunciation": "Bada sapna dekho, chhote se shuru karo."},
            {"english": "Your future is created by your actions.", "hindi": "तुम्हारा भविष्य तुम्हारे कर्मों से बनता है।", "pronunciation": "Tumhara bhavishya tumhare karmon se banta hai."},
            {"english": "Never give up on your dreams.", "hindi": "अपने सपनों पर कभी मत हारो।", "pronunciation": "Apne sapnon par kabhi mat haaro."},
        ],
        "Love": [
            {"english": "Love yourself first.", "hindi": "पहले खुद से प्यार करो।", "pronunciation": "Pehle khud se pyaar karo."},
            {"english": "Love makes everything possible.", "hindi": "प्यार सब कुछ संभव बनाता है।", "pronunciation": "Pyaar sab kuch sambhav banata hai."},
        ],
    }
...
def generate_all_audio(phrases: list, output_dir: str):
    ...
    for i, phrase in enumerate(phrases):
        ...
        hi_success = asyncio.run(generate_single_audio(phrase["hindi"], HINDI_VOICE, str(hindi_file)))
        if hi_success:
            print(f"    ✓ Hindi: {hindi_file.name}")
        else:
            cmd = ["ffmpeg", "-y", "-f", "lavfi", "-i", "anullsrc=r=24000:cl=mono", "-t", "2", str(hindi_file)]
            subprocess.run(cmd, capture_output=True)

        # Get ACTUAL durations
        en_duration = get_audio_duration(str(english_file))
        hi_duration = get_audio_duration(str(hindi_file))

        # Add pause between English and Hindi
        pause_between = 0.5
        total_duration = en_duration + pause_between + hi_duration

        print(f"    ⏱️  Total: {total_duration:.2f}s (EN: {en_duration:.2f}s + pause: {pause_between}s + HI: {hi_duration:.2f}s)")
        ...
        if result.returncode != 0:
            ...
            with open(concat_file, "w", encoding="utf-8") as f:
                ...
                f.write(f"file '{hindi_file.as_posix()}'\n")
        ...
        audio_files.append({
            ...
            "hindi": str(hindi_file),
            "combined": str(combined_file),
            "duration": actual_duration,
            "en_duration": en_duration,
            "hi_duration": hi_duration
        })
...
def generate_complete_image(phrase_data: dict, category_english: str, output_path: str):
    ...
    draw.rectangle(
        [(60, hindi_y - 55), (VIDEO_WIDTH - 60, hindi_y + total_height + 15)],
        fill=(80, 30, 30, 220)
    )

    for i, line in enumerate(hindi_lines):
        y_pos = hindi_y + (i * 95)  # Increased spacing
        draw.text(
            (VIDEO_WIDTH // 2, y_pos),
            line,
            fill=(255, 255, 0),
    ...
    pronunciation_y = hindi_y + total_height + 90  # Increased from 80
...
def generate_reel(category_english: str = None):
    ...
    print(f"\n{'='*80}")
    print(f"Category: {category_english} ({CATEGORIES_HINDI[category_english]})")
    print(f"{'='*80}\n")
    ...
    print(f"  🏷️  Branding: Velocity Hindi")
    print(f"{'='*80}\n")
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("🇮🇳 VELOCITY HINDI - FACEBOOK REELS AUTOMATION 🇮🇳")
    print("="*80)
    print("\n✨ IMPROVED FEATURES:")
    ...
    print("  ✓ English category names (for English speakers learning Hindi)")
    print("  ✓ Velocity Hindi branding at bottom")
